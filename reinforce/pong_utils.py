...
def play(env, policy, time=2000, preprocess=None, nrand=5):
    ...
    for _ in range(nrand):
        step_result1 = env.step(np.random.choice([RIGHT,LEFT]))
        frame1, _, is_done = step_result1[:3]
        step_result2 = env.step(0)
        frame2, _, is_done = step_result2[:3]
...
def clipped_surrogate(policy, old_probs, states, actions, rewards,
    ...
    clipped_surrogate_loss = torch.min(ratio*rewards, clip*rewards)
    ...
    return torch.mean(clipped_surrogate_loss + beta*entropy)
